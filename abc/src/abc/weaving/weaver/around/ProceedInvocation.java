...
public class ProceedInvocation {
	...
	public void generateProceed(ProceedMethod proceedMethod, String newStaticInvokeClassName) {
		...
		Stmt levelUpStmt = Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(Scene.v().getMethod("<abc.reentrant.runtime.Level: void forceUnmanagedUp1()>").makeRef())); //ok??
		Stmt levelUpStmt2 = Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(Scene.v().getMethod("<abc.reentrant.runtime.Level: void forceUnmanagedUp1()>").makeRef())); //ok??
		Stmt levelDownStmt = Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(Scene.v().getMethod("<abc.reentrant.runtime.Level: void forceUnmanagedDown1()>").makeRef())); //ok??
		statements.insertAfter(levelDownStmt, this.begin);
		statements.insertBefore(levelUpStmt, this.end);
if (this.dynamicInvoke != null) {statements.insertBefore(levelUpStmt2, this.end);}
