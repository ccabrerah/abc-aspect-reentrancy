...
import org.aspectbench.runtime.LCFlow;

import java.util.Stack;
import java.util.Hashtable;
import java.util.Enumeration;

public class LCFlowStack {
	...
	/*	if (stack_traces < MAX_STACK_TRACES) {
        ...
        getThreadStack().push(new LCFlow(obj));
    }
	...
	/*	if (stack_traces < MAX_STACK_TRACES) {
        ...
        getThreadStack().push(new LCFlowPlusState(obj));
    }
    ...
    public LCFlow peekLCFlow() {
        Stack stack = getThreadStack();
        if (stack.isEmpty()) return null;
        return (LCFlow)stack.peek();
    }

    public LCFlow peekTopLCFlow() {
        Stack stack = getThreadStack();
        if (stack.isEmpty()) return null;
        return (LCFlow)stack.elementAt(0);
    }
    ...
    public static final void pushInstanceStack(Object obj, Object stack) {
        ((Stack)stack).push(new LCFlow(obj));
    }

    public static final void pushStack(Object[] obj, Object stack) {
        ((Stack)stack).push(new LCFlowPlusState(obj));
    }
    ...
    public static final Object getTopStack(int index, Object stack) {
        LCFlow cf = peekLCFlowStack(stack);
        return (null == cf ? null : cf.get(index));
    }
    ...
    public static final LCFlow peekLCFlowStack(Object stack) {
        if (((Stack)stack).isEmpty()) return null;
        return (LCFlow)((Stack)stack).peek();
    }

    public static final LCFlow peekTopLCFlowStack(Object stack) {
        if (((Stack)stack).isEmpty()) return null;
        return (LCFlow)((Stack)stack).elementAt(0);
    }
